package com.marketflow.identity.infrastructure.messaging;
...
public final class OutboxPublisher {
    ...
    private void send(PendingEvent event) {
        kafka.send("marketflow.identity.events.v1", event.aggregateId().toString(), event.payload())
                .whenComplete(
                        (result, failure) -> {
                            if (failure == null) {
                                jdbc.update(
                                ...
                                LOGGER.atWarn()
                                        ...
                                        .log("Identity outbox publication deferred");
                            }
                        });
    }
