...
| Notification | Templates, delivery attempts, retries, DLQs | PostgreSQL; RabbitMQ tasks |

The sample service is a foundation health/telemetry example, not a business bounded context.
Services do not query one another's databases or share persistence entities.

## Event-Driven Architecture

Kafka carries durable, versioned domain facts such as product publication, reservations, order state, and payment outcomes. RabbitMQ carries task commands such as notification delivery, where explicit acknowledgement, retry routing, and dead-letter queues are useful. Events use correlation IDs, versioned payloads, and idempotent consumers. See [events and messages](docs/events/README.md).
...
See the [checkout Saga guide](docs/architecture/checkout-saga.md).
...
Identity, Seller, Catalog, Inventory, Order, Payment, and Notification each own a separate PostgreSQL database. Cart owns Redis documents; Search owns only its rebuildable OpenSearch projection. Cross-service identifiers are opaque UUIDs and integration uses APIs/events rather than cross-database joins. See [data ownership](docs/architecture/data-ownership.md).

## Security

Passwords are hashed; access tokens are short-lived; refresh tokens rotate and can be revoked. Role and resource-ownership checks are enforced in services, including seller isolation. Only fake opaque payment tokens are accepted. Card numbers, real provider credentials, and secrets are not collected or committed. See the [security guide](docs/security/README.md).

## Observability

Services emit structured logs with correlation IDs, Prometheus metrics, and OpenTelemetry traces. Health/readiness endpoints, dashboards, alert rules, queue/outbox signals, and operational runbooks are included. See the [observability guide](docs/architecture/observability.md).

## Testing

The repository includes unit, integration, migration, contract, concurrency, idempotency, Saga, chaos, backup/restore, frontend, and Playwright tests. Verified evidence and environment limits are recorded in the [release-candidate report](docs/release/release-candidate-report.md). No metrics, test counts, latency, throughput, uptime, or security claims are inferred beyond recorded evidence.

## Local Kubernetes

Docker Compose is the reference local runtime. Kubernetes manifests and a Helm chart target Docker Desktop Kubernetes, kind, k3d, or another local cluster using placeholder configuration and out-of-band Secrets. No paid managed service is required. See the [local Kubernetes guide](docs/deployment/local-kubernetes.md).
