...
$smokeCorrelationId = 'm0-infrastructure-smoke'
...
if ($LASTEXITCODE -ne 0) { throw 'Kafka readiness check failed.' }
...
$sampleLogs = (& docker compose logs --no-color --tail 200 sample-service) -join [Environment]::NewLine
if ($LASTEXITCODE -ne 0 -or $sampleLogs -notmatch '"correlationId":"m0-infrastructure-smoke"') {
    throw 'The structured sample-service log did not contain the smoke correlation ID.'
}
Write-Host 'PASS structured correlation log'

Write-Host 'All MarketFlow Milestone 0 infrastructure smoke checks passed.'
